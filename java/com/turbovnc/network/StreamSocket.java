...
public class StreamSocket extends Socket {
  ...
  public StreamSocket(InputStream in, OutputStream out, boolean own) {
    instream = new InputStreamInStream(in);
    outstream = new OutputStreamOutStream(out);
    ownStreams = own;
  }
  ...
  public void shutdown() {
    super.shutdown();
    try {
      ((InputStreamInStream)instream).getStream().close();
      ((OutputStreamOutStream)outstream).getStream().close();
